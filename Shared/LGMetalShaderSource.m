...
NSString *LGGlassMetalSource(void) {
    static NSString * const source = @"// fullscreen quad + glass shading\n"
        ...
        "    float specular = specLobe * strokeMask * u.specularOpacity * (useShapeMask ? 1.05 : 1.45) * edgeOpacity;\n"
        "    float highlight = specular + fresnel * (useShapeMask ? 0.24 : 0.42);\n"
        "    float3 lch = srgbToLch(clamp(bgColor.rgb, 0.0, 1.0));\n"
        "    lch.x = clamp(lch.x + highlight * (useShapeMask ? 15.0 : 26.0), 0.0, 100.0);\n"
        "    lch.y = max(0.0, lch.y - highlight * (useShapeMask ? 3.5 : 6.5));\n"
        "    float3 shapedHighlight = lchToSrgb(lch);\n"
        "    bgColor.rgb = mix(bgColor.rgb, shapedHighlight, clamp(highlight * (useShapeMask ? 0.42 : 0.55), 0.0, 1.0));\n"
